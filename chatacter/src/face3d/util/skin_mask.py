...
class GMM:
    ...
    def __init__(self, dim, num, w, mu, cov, cov_det, cov_inv):
        ...
        self.cov = (
            # covariance matrix of Gaussian components (a list of dimxdim matrices)
            cov
        )
        # pre-computed determinet of covariance matrices (a list of scalars)
        self.cov_det = cov_det
        # pre-computed inverse covariance matrices (a list of dimxdim matrices)
...
gmm_nonskin = GMM(
    3, 4, gmm_nonskin_w, gmm_nonskin_mu, [], gmm_nonskin_cov_det, gmm_nonskin_cov_inv
)
