...
def main(args):
    ...
    img_source = img_preprocessing(args.test_image_path, args.image_size).to(
        args.device
    )
    ...
    if os.path.exists(args.pose_driven_path):
        ...
        pose_signal = (
            torch.Tensor(pose_obj).unsqueeze(0).to(args.device) / 90
    ...
    face_location_signal = (
        torch.zeros(1, frame_end, 1).to(args.device) + args.face_location
    )
    ...
    for pred_index in tqdm(range(generated_directions.shape[1])):
        ori_img_recon = lia.render(
            one_shot_lia_start,
            torch.Tensor(generated_directions[:, pred_index, :]).to(args.device),
            feats,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--infer_type",
        type=str,
        default="mfcc_pose_only",
        help="mfcc_pose_only or mfcc_full_control",
    )
    parser.add_argument(
        "--test_image_path",
        type=str,
        default="./test_demos/portraits/monalisa.jpg",
        help="Path to the portrait",
    )
    parser.add_argument(
        "--test_audio_path",
        type=str,
        default="./test_demos/audios/english_female.wav",
        help="Path to the driven audio",
    )
    parser.add_argument(
        "--test_hubert_path",
        type=str,
        default="./test_demos/audios_hubert/english_female.npy",
        help="Path to the driven audio(hubert type). Not needed for MFCC",
    )
    parser.add_argument(
        "--result_path", type=str, default="./results/", help="Type of inference"
    )
    parser.add_argument(
        "--stage1_checkpoint_path",
        type=str,
        default="./ckpts/stage1.ckpt",
        help="Path to the checkpoint of Stage1",
    )
    parser.add_argument(
        "--stage2_checkpoint_path",
        type=str,
        default="./ckpts/pose_only.ckpt",
        help="Path to the checkpoint of Stage2",
    )
    parser.add_argument("--seed", type=int, default=0, help="seed for generations")
    parser.add_argument(
        "--control_flag",
        action="store_true",
        help="Whether to use control signal or not",
    )
    parser.add_argument(
        "--pose_yaw",
        type=float,
        default=0.25,
        help="range from -1 to 1 (-90 ~ 90 angles)",
    )
    parser.add_argument(
        "--pose_pitch",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="range from 0 to 1 (from left to right)",
    )
    parser.add_argument(
        "--pose_driven_path",
        type=str,
        default="xxx",
        help="path to pose numpy, shape is (T, 3). You can check the following code https://github.com/liutaocode/talking_face_preprocessing to extract the yaw, pitch and roll.",
    )
    parser.add_argument(
        "--face_scale",
        type=float,
        default=0.5,
        help="range from 0 to 1 (from small to large)",
    )
    parser.add_argument(
        "--step_T", type=int, default=50, help="Step T for diffusion denoising process"
    )
    parser.add_argument(
        "--image_size", type=int, default=256, help="Size of the image. Do not change."
    )
    parser.add_argument(
        "--device", type=str, default="cuda:0", help="Device for computation"
    )
    parser.add_argument(
        "--motion_dim", type=int, default=20, help="Dimension of motion. Do not change."
    )
    parser.add_argument(
        "--decoder_layers", type=int, default=2, help="Layer number for the conformer."
    )
    parser.add_argument(
        "--face_sr",
        action="store_true",
        help="Face super-resolution (Optional). Please install GFPGAN first",
    )
